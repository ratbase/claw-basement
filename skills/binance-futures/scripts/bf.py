...
Usage:
  ...
  ./bf.py journal add SYM DIR SIZE @PRICE [Nx] [notes]   Add trade to journal
  ./bf.py journal list [--open] [--days N]                List trades
...
def cmd_account(key: str, secret: str, raw: bool = False):
    ...
    print(
        f"  Total:             {colored(upnl, f'{upnl:>+13,.2f} USDT')}  {pnl_dot(upnl)}"
    )
...
def cmd_positions(key: str, secret: str, raw: bool = False):
    ...
    total_upnl = 0.0
    for p in sorted(
        open_pos, key=lambda x: abs(float(x["unRealizedProfit"])), reverse=True
    ):
...
def cmd_income(
    ...
    for rec in all_records:
        ...
        month = datetime.fromtimestamp(
            int(rec["time"]) / 1000, tz=timezone.utc
        ).strftime("%Y-%m")
        ...
        if itype == "REALIZED_PNL":
            sym_stats[sym]["pnl"] += amt
        elif itype == "FUNDING_FEE":
            sym_stats[sym]["funding"] += amt
        elif itype == "COMMISSION":
            sym_stats[sym]["commission"] += amt
    ...
    print(
        f"{DIM}{'Last ' + str(days) + ' days  ·  ' + str(len(all_records)) + ' records':^72}{RESET}"
    )
    sep()

    print(f"\n{YEL}📊 Summary{RESET}")
    print(
        f"  Realized PnL:    {colored(total_pnl, f'{total_pnl:>+13,.2f} USDT')}  {pnl_dot(total_pnl)}"
    )
    print(
        f"  Funding Fees:    {colored(total_fund, f'{total_fund:>+13,.2f} USDT')}  {pnl_dot(total_fund)}"
    )
    ...
    for itype, amt in sorted(
        type_totals.items(), key=lambda x: abs(x[1]), reverse=True
    ):
...
def cmd_trades(
    ...
    print(
        f"{DIM}{'Last ' + str(days) + ' days  ·  ' + str(len(data)) + ' trades':^72}{RESET}"
    )
    sep()
...
def cmd_rr(
    ...
    if balance:
        ...
        print(
            f"  Margin Required:  {margin:,.2f} USDT  ({margin_pct:.1f}% of account @ {leverage}x)"
        )

    print(f"\n{YEL}🎯 Take Profit Levels{RESET}")
    r_levels = (
        [(target_r, "Custom")] if target_r else [(1.0, "1R"), (2.0, "2R"), (3.0, "3R")]
    )
...
def _journal_path() -> str:
    return os.path.expanduser("~/.picoclaw/workspace/state/trades.json")
...
def cmd_journal_list(show_open: bool = False, days: int = 30):
    ...
    if not trades:
        ...
        return
    sep()
    print(f"{BOLD}{'TRADING JOURNAL':^72}{RESET}")
    sep()
    ...
    print(f"  Showing {len(filtered)} of {len(trades)} trades (last {days} days)")
    print()
    for t in filtered:
        status_c = GREEN if t["status"] == "OPEN" else DIM
...
def cmd_klines(
    ...
    if struct == "bull_trend":
        regime = "BULL"
    elif struct == "bear_trend":
        regime = "BEAR"
    else:
        regime = "RANGE"
    if adx_ok and adx < 20:
        regime = "RANGE"
    ...
    if adx_anomaly:
        ...
        if adx >= 25:
            adx_disp += f"  {GREEN}trending{RESET}"
        elif adx <= 20:
            adx_disp += f"  {YEL}ranging{RESET}"
        else:
            adx_disp += f"  {DIM}transitional{RESET}"
    ...
    if ema20:
        print(f"  EMA 20:      {ema20:>16,.4f}  (price {pve20})")
    if ema50:
        print(f"  EMA 50:      {ema50:>16,.4f}  (price {pve50})")
    print(f"  ADX (14):    {adx_disp}")
    if plus_di and minus_di:
        bias = (
            f"{GREEN}bullish{RESET}" if plus_di > minus_di else f"{RED}bearish{RESET}"
        )
...
commands:
  ...
  journal         Add/list trades to journal
...
examples:
  ...
  ./bf.py journal list --open
    ...
    p.add_argument(
        "--type", dest="income_type", help="REALIZED_PNL | FUNDING_FEE | COMMISSION"
    )
    p.add_argument("--raw", action="store_true", help="Raw JSON")
    ...
    p.add_argument(
        "symbol", nargs="?", default="BTCUSDT", help="Symbol (default: BTCUSDT)"
    )
    p.add_argument("--interval", default="4h", help="Candle interval (default: 4h)")
    p.add_argument(
        "--limit", type=int, default=100, help="Number of candles (default: 100)"
    )
    ...
    p.add_argument(
        "--risk", type=float, default=2, help="Risk %% per trade (default: 2)"
    )
    p.add_argument("--leverage", type=int, default=10, help="Leverage (default: 10)")

    # journal
    p = sub.add_parser("journal", help="Trading journal — add or list trades")
    subp = p.add_subparsers(dest="jcmd", metavar="CMD")
    a = subp.add_parser("add", help="Add a trade")
    a.add_argument("symbol", help="Symbol, e.g. BTCUSDT")
    a.add_argument("direction", help="LONG or SHORT")
    a.add_argument(
        "size", type=float, help="Position size in base currency (e.g. 0.05 BTC)"
    )
    ...
    if args.cmd == "rr":
        cmd_rr(
            args.entry, args.stop, args.target, args.balance, args.risk, args.leverage
        )
        return
    if args.cmd == "klines":
