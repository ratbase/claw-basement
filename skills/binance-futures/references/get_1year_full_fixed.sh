...
while [ "$CURRENT_START" -lt "$NOW" ]; do
    ...
    QUERY_STRING="endTime=$NOW&limit=1000&startTime=$CURRENT_START&timestamp=$TIMESTAMP"

    # Generate signature
    SIGNATURE=$(echo -n "$QUERY_STRING" | openssl dgst -sha256 -hmac "$BINANCE_API_SECRET" | awk '{print $2}')

    URL="$API_BASE/fapi/v1/income?$QUERY_STRING&signature=$SIGNATURE"

    RESULT=$(curl -s -H "X-MBX-APIKEY: $BINANCE_API_KEY" "$URL")
